...
st_data = time.time()
new_lineitem_df= dw.lineitem.project(('l_quantity', 'l_extendedprice')).filter(Q('l_quantity',14,CMP.LT))
en_1 = time.time()
print(str(float(time.time())))
...
def trainingLoop(dw):
    ...
    cl, c = k_means(dw,x, c, cl, K, Niter=Niter, if_gpu =False)
    dw.cl = cl
    dw.c = c
    end = time.time()
