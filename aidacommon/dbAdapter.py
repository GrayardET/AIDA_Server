...
class DBC(metaclass=ABCMeta):
    ...
    def _job(self,iter_func,cond_func,test_func,name,*args,**kwargs):
        ...
        #put it directly to GPU queue with estimate time
            ...
            with condition:
                self._schMgr.insert_GPU(condition,name,self,estimate_time,GPU_test[2]);
                condition.wait();
        else:
            ...
            if(epoch_total != LARGE_NUM):
                ...
                logging.info("cpu"+ str(exec_on_CPU_time))
                logging.info("gpu"+ str(exec_on_GPU_time))
                if(float(exec_on_GPU_time/exec_on_CPU_time) > 1):
                    logging.info("need to go cpu queue")
                    self._schMgr.insert_CPU(condition,name,self,estimate_time_CPU,CPU_test[2]);
                else:
                    logging.info("need to go gpu queue")
                    self._schMgr.insert_GPU(condition,name,self,estimate_time_GPU,GPU_test[2]);
            else:
                #unknown
                if(float(CPU_test[1]/CPU_test[0]) > float(GPU_test[1]/GPU_test[0])):
                    self._schMgr.insert_CPU_long(condition,name,self,CPU_test[2]);
                else:
                    self._schMgr.insert_GPU_long(condition,name,self,GPU_test[2]);
            self._schMgr.finish_test_gpu();
